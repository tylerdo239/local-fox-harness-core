...
// Exported for composer.tsx's Dừng/Tiếp tục/Huỷ bỏ task flow — the seq of
// the most recent `turn/start` lets it tell "the turn I just told the agent
// to stop" apart from "a genuinely new turn started since".
//
// REAL, CONFIRMED UPSTREAM BUG (not this app's code) this exists to work
// around: a turn cancelled via `agent.cancel()` never actually gets a
// `turn/end` appended, so `runningState(events).running` below would stay
// stuck `true` forever after Dừng/Huỷ bỏ task with no other signal to tell
// composer.tsx the agent moved on. Traced end to end with temporary console
// instrumentation directly in `node_modules/@deepseek-ai/dsh-agent-loop`
// (reverted after, not shipped): `ReactLoopAgent.turn()`'s own catch block
// correctly computes `turnEnds = {kind:'aborted', reason: signal.reason}`
// and reaches its `finally`, but `this.session.append('turn/end', {turn,
// reason: turnEnds})` itself THROWS — `signal.reason` (dsh-session's own
// frozen `AgentCancelCause`) is a null-prototype object, and dsh-session's
...
// Not fixable from this app (it's inside a 3rd-party dependency).
